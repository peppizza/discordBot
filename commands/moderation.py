...
class Moderation(commands.Cog):
    ...
    async def on_member_join(self, member):
        guild = member.guild
        if guild.id != 684472795639447621:
            return
        ...
        embed = discord.Embed(title='Welcome {}!'.format(
            member.name), description='Head to {} for the rules,\n{} for self-promotion,\nand feel free to make a suggestion!'.format(rules.mention, promotion.mention))
    ...
    async def warn(self, ctx, member: discord.Member, *args, auto=False):
        ...
        embed.add_field(name='you have been warned by',
                        value=ctx.message.author)
        embed.add_field(name='reason', value=' '.join(args[0:]))
        if not ctx.author.bot:
            await member.send(embed=embed)
        if auto == False:
            embed = discord.Embed(title='{} has been warned'.format(
                member), description='**{}** has been warned by **{}**'.format(member, ctx.author))
            embed.add_field(name='Reason', value=' '.join(args[0:]))
        else:
            embed = discord.Embed(title='{} has been warned'.format(
                member), description='**{}** has been automatically warned for hate speech'.format(member))
